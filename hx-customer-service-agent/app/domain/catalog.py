...
KNOWLEDGE_BASE: tuple[KnowledgeItem, ...] = (
    KnowledgeItem(
        ...
        intents=("business_hours", "faq"),
    ),
    KnowledgeItem(
        collection="products",
        title="菜品口味",
        content="默认口味偏清淡，可备注少辣、不辣、少油；过敏信息建议在下单备注中明确填写。",
        intents=("product_inquiry", "recommendation", "taste_inquiry"),
    ),
    KnowledgeItem(
        collection="policies",
        title="退款规则",
        content="未出餐订单可申请取消；已出餐订单需按错发、漏发、食品质量等原因提交售后。",
        intents=("refund_request", "order_cancel", "policy_inquiry"),
    ),
    KnowledgeItem(
        collection="policies",
        title="食品安全处理",
        content="疑似异物、过敏或身体不适属于 P0 场景，应保留照片和订单信息，并优先转人工跟进。",
        intents=("food_safety", "complaint"),
    ),
    KnowledgeItem(
        collection="faq",
        title="配送查询",
        content="配送状态可通过订单号查询；若预计送达时间异常，可发起催单通知商家和骑手。",
        intents=("delivery_inquiry", "order_rush"),
    ),
)
